...
import {computed, inject, isReactive, reactive, toRaw} from "vue";
import VFlexTableCell from "./VFlexTableCell.vue";

export default {
    components: {
        flewTableWrapperSymbol,
        VFlexTableCell
    },
    props: {
        data: {
        ...
        <slot name="body">
            <template v-for="(row, index) in data" :key="index">
                ...
                <div class="flex-table-item"
                     ...
                     @keydown.space.prevent="() => {props.clickable && context.emit('rowClick', row, index) }"
                     @click=" () => { props.clickable && context.emit('rowClick', row, index)  }">
                        ...
                        <template v-for="column in columns" :key="'row' + column.key">
                            <VFlexTableCell :column="column">
                                ...
                                >
                                    ...
                                    ></component>
                                    <span
                                        v-else-if="typeof column.format(row[column.key], row, index) === 'object'"
