    ...
    export default {
        ...
        methods: {
            ...
            async fetchMoreRooms() {
                ...
                const roomUserIds = []
                data.forEach(room => {
                    console.log( 'room.users22' ,  room ,  room.users);
                ...
                roomUserIds.forEach(userId => {
                    const promise = firestoreService.getUserData('7WLqg8jVGiDh0Wy8R8Un' , 'user-auth-100')
                ...
                Object.keys(roomList).forEach(key => {
                    ...
                    firestoreService.getAllUsersData(key)
                        .then(({data, docs}) => {
                         let roomObj =   _.find(this.rooms,{roomId: key});

                         if(roomObj)
                             roomObj.roomName = data.filter(
                                user => user.userId !== this.currentUserId
                            ).map(user => user.userName).join(', ')|| 'Myself'
                            // data.forEach(user => {
                            //
                            // })
                            // room.roomName =  " ssss"
                            // alert("ssss")
                            // room.roomName = data.map(user => user.userName).join(', ') || 'Myself'
                            // console.log("7WLqg8jVGiDh0Wy8R8Un",roomObj, data, docs)
            ...
            async createRoom() {
                ...
                if (roomData && roomData.id)
                    await firestoreService.addUserData(roomData.id, this.currentUserId, this.userData)
