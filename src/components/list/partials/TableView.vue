    ...
    <div class="flex-table flex-table-custom">
        ...
        <div class="flex-table-header">
            <slot name="table_header" :column="column">
                <span v-if="selectable" class="flex-datatable-cell cell-start checkbox-selector">
                        <label class="checkbox is-primary is-outlined is-circle">
                            <input :id="`head-checkbox-${resource}`" name="allSelected" value="1" @change="onSelectAllChange" type="checkbox">
                            <span></span>
                        </label>
                </span>
                        ...
                        <div v-for="column in columns" class="flex-table-cell" :data-th="column.text">
                            <slot :name="`column.${column.value}`" :row="row" :column="column"
                                ...
                                <component v-if="column.component" :is="`${column.component}`"
                                           ...
                                           :row="row"/>
                                <span v-else
                                      :class="[ column.bold && 'has-dark-text dark-inverted is-font-alt is-weight-600 rem-90',
